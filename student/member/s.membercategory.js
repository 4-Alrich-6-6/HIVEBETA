...
const loadProjectPermissions = async () => {
    ...
    const { data: membership } = await supabase
        ...
        .eq("userId", user.id)
        .maybeSingle();
    canCreateProjects = String(membership?.ROLE?.roleName || "").trim().toLowerCase() === "leader";
    if (openPostCategoryModalBtn) openPostCategoryModalBtn.hidden = !canCreateProjects;
};
...
if (groupInfoTab)  groupInfoTab.addEventListener("click",  () => {
    const grpId = getGrpId();
    window.location.href = `s.membergrpviewing.html${grpId ? "?grpId=" + grpId : ""}`;
});
