(() => {
    ...
    const loadStats = async (member) => {
        ...
        const { data: currentAssignments } = currentMemberIds.length
            ? await supabase.from("TASKASSIGNMENT").select("taskId").in("grpmemId", currentMemberIds)
            : { data: [] };
        ...
        const averageScore = taskHistory.length ? Math.round((completed / taskHistory.length) * 100) : null;
